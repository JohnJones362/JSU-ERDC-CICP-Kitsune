...
def attempt_file_delete(file_path, max_retries=5, retry_delay=0.5):
    ...
    for i in range(max_retries):
        if os.path.exists(file_path):
            try:
                os.remove(file_path)
                print(f"Successfully deleted existing file: {file_path}")
                return True
            except PermissionError as e:
                print(f"Attempt {i+1}/{max_retries}: Could not delete '{file_path}' due to permission error: {e}. Retrying in {retry_delay}s...")
                time.sleep(retry_delay)
            except Exception as e:
                print(f"Error during file deletion of '{file_path}': {e}")
                raise # Re-raise other exceptions
        else:
            print(f"File '{file_path}' does not exist. No need to delete.")
            return True # File doesn't exist, so it's clean

    print(f"Failed to delete '{file_path}' after {max_retries} attempts. Please close any programs using this file.")
...
def wait_for_file_to_exist(file_path, timeout=10, check_interval=0.1):
    ...
    print(f"File '{file_path}' found after {time.time() - start_time:.2f} seconds.")
    return True
...
def main():
    ...
    if run_realtime_simulation:
        if not attempt_file_delete(OUTPUT_FILE):
            sys.exit(1) # Exit if we can't delete the file, as simulation won't work

    if run_realtime_simulation:
        print("Starting real-time data simulation...")
        try:
            # Start the simulation script as a separate process
            # Note: realtime_data_simulator.py no longer handles its own deletion
            realtime_sim_process = subprocess.Popen([sys.executable, 'realtime_data_simulator.py'])
            print(f"Giving simulator {SLEEP_TIME * 2} seconds to start writing data and ensure file creation...")
            # Wait for the file to actually exist before proceeding with resource checks or Kitsune initialization
            if not wait_for_file_to_exist(OUTPUT_FILE, timeout=10):
                print(f"Error: Simulator did not create '{OUTPUT_FILE}' in time. Aborting.")
                if realtime_sim_process:
                    realtime_sim_process.terminate()
                sys.exit(1)

            print("Simulator should be running and file created.")
    ...
    start_time_overall = time.time() # Changed 'start' to 'start_time_overall' for clarity
    ...
    try:
        if run_realtime_simulation:
            ...
            # Set a starting time for rate calculation within the loop
            ...
            while True:
                ...
                if RMSE == -2:
                    if packet_count % 1000 == 0:
                        elapsed_time = time.time() - loop_start_time # Use loop_start_time for more consistent rate during grace period
                        if elapsed_time > 0:
                            processing_rate = packet_count / elapsed_time
                            print(f"PCA training in progress... Packet {packet_count}, current rate: {processing_rate:.2f} packets/sec")
                        else:
                ...
                if packet_count > total_grace_packets and RMSE is not None and RMSE >= 0:
                    ...
                    if packet_count % 1000 == 0:
                        # Modified progress print for real-time
                        elapsed_time = time.time() - loop_start_time
                        if elapsed_time > 0:
                            processing_rate = packet_count / elapsed_time
                            print(f"Processed {packet_count} packets. Latest RMSE: {RMSE:.4f}. Rate: {processing_rate:.2f} packets/sec")
                        else:
            ...
            stop = time.time()
            print("\nComplete. Time elapsed: "+ str(stop - start_time_overall))
            ...
            with open(log_file, "w") as f:
                ...
                f.write(f"- Time elapsed: {time.time() - start_time_overall:.2f} seconds\n")
                f.write(f"- Processing rate: {len(RMSEs)/(time.time() - start_time_overall):.2f} packets/second\n")
