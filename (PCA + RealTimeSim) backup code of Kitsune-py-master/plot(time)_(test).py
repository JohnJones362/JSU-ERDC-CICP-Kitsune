...
# Prepare data for plotting within the attack time frame
start_time_plot = datetime.datetime(2025, 5, 5, 10, 5, 0)
end_time_plot = datetime.datetime(2025, 5, 5, 13, 45, 0)
...
rmse_to_plot = [val if val > 0 else 1e-6 for val in rmse_to_plot]

# Create the plot
plt.figure(figsize=(16, 8)) # Increased figure size for better time visualization
plt.plot(timestamps_to_plot, rmse_to_plot, color='violet', label='RMSE')

# Plot the threshold line
if threshold is not None:
    plt.axhline(y=threshold, color='green', linestyle='--', linewidth=1, label=f'Threshold (mean + 2*SD)')

# Plot vertical lines for attack times
first_attack = True
for attack_time in attack_times:
    if start_time_plot <= attack_time <= end_time_plot:
        plt.axvline(x=attack_time, color='orange', linestyle=':', linewidth=0.75,
                    label='Attack' if first_attack else "")
        first_attack = False

# Customize the plot
plt.yscale("log")
plt.title("RMSE Anomaly Scores Over Time")
